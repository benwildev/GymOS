...
import { PrintButton } from "@/app/dashboard/payments/[id]/receipt/components/PrintButton";
import { auth } from "@/lib/auth";

export const metadata = {
  title: "Payment Receipt | GymOS Member",
};

export default async function MemberPaymentReceiptPage(props: {
  ...
  // Security check: Verify member owns this receipt
  if (data.payment.userId !== session.user.id) {
    notFound();
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="print:hidden flex items-center justify-between">
        ...
        >
          <ArrowLeft className="w-3.5 h-3.5" /> Back to My Payments
        </Link>
        <PrintButton />
      </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-6 text-sm">
          <div>
            ...
            <div className="font-bold text-base mt-1">{payment.user?.name || session.user.name || "Member"}</div>
        ...
        <div className="pt-6 border-t text-center space-y-1 text-xs text-muted-foreground">
          <p className="font-medium text-gray-700">Thank you for your payment!</p>
          <p>Official computer-generated receipt issued by {gym?.name || "GymOS"}.</p>
          <p className="text-[10px] text-gray-400">Issued: {new Date().toLocaleString()}</p>
        </div>
      </div>
    </div>
  );
